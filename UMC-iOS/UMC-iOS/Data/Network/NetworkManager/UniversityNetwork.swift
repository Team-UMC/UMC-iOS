...
//            universities = universities.joinUniversities.mapToUniversityList()
    ...
    // University API - 전체 학교 조회 API(fetch)
    @MainActor
    static func fetchGetContributionRankingInCampus() async -> UniversityResponse.GetContributionRankingInCampus {
        var contributionRanking = UniversityResponse.GetContributionRankingInCampus()
        do {
            contributionRanking = try await getContributionRankingInCampus()
            print(contributionRanking)
//            universities = universities.joinUniversities.mapToUniversityList()
        } catch {
            print("Error: \(error)")
        }
    ...
    // University API - 전체 학교 조회 API
    static func getContributionRankingInCampus() async throws -> UniversityResponse.GetContributionRankingInCampus {
        var urlComponents = ApiEndpoints.getBasicUrlComponents()
        ...
        let jsonDictionary = try decoder.decode(BaseResponse<UniversityResponse.GetContributionRankingInCampus>.self, from: data)
        
        print(jsonDictionary)
        
        var contributionRanking: UniversityResponse.GetContributionRankingInCampus
        contributionRanking = jsonDictionary.result
